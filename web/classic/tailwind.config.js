...
export default {
  ...
  theme: {
    extend: {
      // Setting `theme.colors` directly (instead of `theme.extend.colors`)
      // replaces Tailwind's entire default palette, which silently turned
      // every `bg-blue-500` / `text-green-600` / `from-purple-500`-style
      // class across the app into a no-op (no CSS generated). Nesting the
      // semi-color-* bridge under `extend` keeps those additions while
      // restoring the stock palette these classes depend on.
      colors: {
